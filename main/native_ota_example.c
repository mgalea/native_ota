...
static void ota_task(void *pvParameter)
{
    ...
    esp_http_client_config_t config = {
        .url = CONFIG_OTA_FIRMWARE_UPG_URL,
        ...
        if (data_read < 0) {
            ...
            if (image_header_was_checked == false) {
                ...
                if (data_read > sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t)) {
                    ...
                    ESP_LOGI(TAG, "ota_begin succeeded");
                } else {
...
static bool diagnostic(void)
{
    ...
    io_conf.pin_bit_mask = (1ULL << CONFIG_OTA_GPIO_DIAGNOSTIC);
    ...
    bool diagnostic_is_ok = gpio_get_level(CONFIG_OTA_GPIO_DIAGNOSTIC);

    gpio_reset_pin(CONFIG_OTA_GPIO_DIAGNOSTIC);
